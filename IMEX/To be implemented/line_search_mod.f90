...
!Givenaninitialguess x(1:n) for  a  root  in n dimensions,   nd  the  root  by  a  globally
...
!function fmin de ned below.  In this  case try restarting from a di erent  initial  guess.
...
!the convergence criterion on x; STPMX is  the  scaled maximum  step length  allowed  in  line searches.
      ...
      EXTERNAL fmin
      nn=fmin(x) !The vector fvec is  also computed by  this  call. test=0. Test for initial 
      nf=fmin(x)!guess being a root.  Use more stringent test than simply TOLF .
      do  i=1,n
      ...
      do  its=1,MAXITS !Start of  iteration loop.
        ...
        !fdjac above with your own routine.
      ...
      test=0. !Test for  convergence on dx.
      do  i=1,n
...
!n; EPS is  the approximate  square root  of the machine  precision.
     ...
     REAL h,temp,f(NMAX)
      do  j=1,n
        ...
        h=EPS*abs(temp)
        if(h.eq.0.)h=EPS
        x(j)=temp+h !Trick  to reduce  nite precision error.
        h=x(j)-temp
        call funcv(n,x,f)
        x(j)=temp
        do  i=1,n !Forward di erence formula.
          df(i,j)=(f(i)-fvec(i))/h
        enddo 
      enddo 
      return
...
!should  check  whether  the  convergence  is  spurious. Parameters: ALFensures  sufcient  decrease  in  function  value;
!TOLX is  the  convergence criterion  on x.
      ...
      do  i=1,n
        slope=slope+g(i)*p(i)
      enddo 
      if(slope.ge.0.) write(*,*) 'roundoff problem in lnsrch'
      test=0. !Compute min.
      do i=1,n
      ...
      enddo 
      alamin=TOLX/test
      alam=1. !Always try full Newton step  rst.
  1   continue !Start of iteration loop.
      ...
      if(alam.lt.alamin)then !Convergence  on x.  For  zero   nding,the calling program should verify the convergence.
        ...
        check=.true.
        return
      else if(f.le.fold+ALF*alam*slope)then !Sucient function decrease.
        return
      else !Backtrack.
