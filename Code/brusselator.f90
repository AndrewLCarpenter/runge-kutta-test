...
!------------------------------------------------------------------------------
      ...
      Program_Step_Select: select case(programstep)
        ...
        case('SET_INITIAL_CONDITIONS')
          ...
          !**Exact Solution** 
          open(unit=39,file='./Exact_Data/exact.brusselator4.data')
        ...
        case('BUILD_JACOBIAN')
          choose_Jac_type: select case(temporal_splitting)
            ...
            case('FIRK') ! For fully implicit schemes
              xjac(1,1) = (   2.0_wp*uvec(1)*uvec(2)/ep-bb-1.0_wp)
              xjac(1,2) = (          uvec(1)*uvec(1)/ep)
              xjac(2,1) = (bb-2.0_wp*uvec(1)*uvec(2)/ep)
              xjac(2,2) = (         -uvec(1)*uvec(1)/ep)
