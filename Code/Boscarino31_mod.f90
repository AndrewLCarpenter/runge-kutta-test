...
!        dt = 0.000025_wp*0.1_wp/10**((iDT-1)/20.0_wp) ! explicit timestep
        dt = 0.2_wp/10**((iDT-1)/20.0_wp)     ! implicit timestep
!        dt=1.0_wp        
        ! Set IC's
...
!        call Bosc_dUdt(uvec,test,ep,dt)
        
!        print*,'----u-----',test(1:vecl:2)
!        print*,'----v-----',test(2:vecl:2)
!        print*,'x',x
!        print*,'dudt',(-1.0_wp)*a*two*pi*cos(two*pi*x(:))+ep*two*two*pi*pi*(a**2-1)*sin(two*pi*x(:))
!        print*,'dvdt',(-1.0_wp)*(a**2)*two*pi*cos(two*pi*x(:))
!        print*,'diff u----=',test(1:vecl:2)-((-1.0_wp)*a*two*pi*cos(two*pi*x(:))+ &
!       &   ep*two*two*pi*pi*(a**2-1)*sin(two*pi*x(:)))
!        print*,'diff v----=',test(2:vecl:2)-(-1.0_wp)*(a**2)*two*pi*cos(two*pi*x(:))
!        print*,maxval(abs(test(1:vecl:2)-dt*((-1.0_wp)*a*two*pi*cos(two*pi*x(:))+ &
!       &   ep*two*two*pi*pi*(a**2-1)*sin(two*pi*x(:)))))
!        print*,maxval(abs(test(2:vecl:2)-dt*(-1.0_wp)*(a**2)*two*pi*cos(two*pi*x(:))))
